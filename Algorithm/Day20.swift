...
extension Day20 {
    /*
     ...
     # ✅ 문제: 가장 긴 K개의 서로 다른 문자로 이루어진 부분 문자열

     문자열 `s`와 정수 `k`가 주어졌을 때, **서로 다른 문자 수가 정확히 k개 이하인 가장 긴 연속 부분 문자열의 길이**를 구하시오.
...
extension Day20 {
    /*
     ...
     # ✅ 문제: 가장 긴 K개의 서로 다른 문자로 이루어진 부분 문자열 (반드시 K개)

     문자열 `s`와 정수 `k`가 주어질 때, **서로 다른 문자 수가 정확히 K개인 가장 긴 연속 부분 문자열의 길이**를 구하시오.
     단, K개 "이하"가 아니라 **"정확히" K개**여야 함.
